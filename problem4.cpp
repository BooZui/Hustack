...
TNODE *findKey(TNODE *root, char key)
{
  ...
  while (child != NULL) {
    TNODE *result = findKey(child, key);
...
void deleteABranch(TNODE *root, char branchRootLabel)
{
  ...
  TNODE *toDelete = NULL;
  TNODE *child = parent->firstChild;

  if (child->info == branchRootLabel) {
    toDelete = child;
    parent->firstChild = child->nextSibling;
  } else {
    TNODE *prev = child;
    child = child->nextSibling;
    while (child != NULL && child->info != branchRootLabel) {
      prev = child;
      child = child->nextSibling;
    }
    if (child != NULL) {
...
void postOrderTraversal(TNODE *root)
{
  ...
  while (!myStack.empty())
  {
    ...
    else
    {
      ...
      { // chua có trong set thì thêm con vào, và thêm vào set
